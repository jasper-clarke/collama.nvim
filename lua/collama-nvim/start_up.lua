...
local ghost_text = require("collama-nvim.ghost_text")
local ollama_client = require("collama-nvim.lsp_client")
...
function M.setup(user_config)
	...
	lspconfig.ollama_lsp.setup({
		...
		on_attach = function(_, bufnr)
			vim.api.nvim_create_user_command(
				"CollamaSuggestion",
				ollama_client.request_completions,
				{ desc = "Get Ollama Suggestion" }
			)
			vim.api.nvim_create_user_command(
				"CollamaAccept",
				ghost_text.accept_first_extmark_lines,
				{ desc = "Accepts displayed Ollama Suggestion" }
			)
			vim.api.nvim_create_user_command(
				"CollamaReject",
				ghost_text.delete_first_extmark,
				{ desc = "Rejects displayed Ollama Suggestion" }
			)
	...
	vim.api.nvim_command("augroup Collama")
	-- Auto commands
	...
	vim.api.nvim_create_user_command("DisableCollama", function()
		disable_plugin()
	end, { desc = "Disables collama.nvim" })

	vim.api.nvim_set_keymap("n", config.keymaps.suggestion, "<Cmd>CollamaSuggestion<CR>", { noremap = true })
	vim.api.nvim_set_keymap("n", config.keymaps.reject, "<Cmd>CollamaReject<CR>", { noremap = true })
